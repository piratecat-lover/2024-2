...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    hidden_layer, hidden_cache = Linear_ReLU.forward(X, W1, b1)
    scores, scores_cache = Linear.forward(hidden_layer, W2, b2)
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers - 1):
      out, cache = Linear_ReLU.forward(out, self.params[f"W{i+1}"], self.params[f"b{i+1}"])
      caches.append(cache)
      if self.use_dropout:
    ...
    # Replace "pass" statement with your code
    loss, dscores = softmax_loss(scores, y)
    loss += self.reg * sum([torch.sum(self.params[f"W{i+1}"]**2) for i in range(self.num_layers)])
    ...
    for i in reversed(range(self.num_layers - 1)):
      ...
      dout, grads[f"W{i+1}"], grads[f"b{i+1}"] = Linear_ReLU.backward(dout, caches.pop())
...
class Dropout(object):
  ...
  def forward(x, dropout_param):
    ...
    if mode == 'train':
      ...
      ###########################################################################
      # Replace "pass" statement with your code
      mask = (torch.rand(x.shape, device=x.device, dtype = x.dtype) > p).float()
      out = x*mask/(1-p)
      ###########################################################################
      #                             END OF YOUR CODE                            #
      ###########################################################################
