...
class Conv(object):
  ...
  def forward(x, w, b, conv_param):
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            for f in range(F):
                out[:, f, i, j] = torch.sum(x_slice * w[f], dim=(1, 2, 3))
  ...
  def backward(dout, cache):
    ...
    # Pad input and gradient wrt input
    x_padded = torch.nn.functional.pad(x, (pad, pad, pad, pad))
    ...
    for i in range(H_out):
        for j in range(W_out):
            x_slice = x_padded[:, :, i*stride:i*stride+HH, j*stride:j*stride+WW]
            for k in range(F):
                dw[k] += torch.sum(x_slice * dout[:, k, i, j][:, None, None, None], dim=0)
            for n in range(N):
                dx_padded[n, :, i*stride:i*stride+HH, j*stride:j*stride+WW] += torch.sum(w[:, :, :, :] * dout[n, :, i, j][:, None, None, None], dim=0)
...
class MaxPool(object):
  ...
  def forward(x, pool_param):
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            out[:, :, i, j] = torch.max(x_slice, dim=(2, 3)).values
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, pool_param)
  ...
  def backward(dout, cache):
    ...
    for i in range(H_out):
        for j in range(W_out):
            x_slice = x[:, :, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width]
            max_x_slice = torch.max(x_slice, dim=(2, 3), keepdim=True).values
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    # Initialize weights and biases for the conv layer
    ...
    # Compute the dimensions after convolution and pooling
    conv_output_dim = H * W * F // 4

    # Initialize weights and biases for the hidden layer
    self.params['W2'] = weight_scale * torch.randn(conv_output_dim, hidden_dim, dtype=dtype, device=device)
  ...
  def loss(self, X, y=None):
    ...
    out, fc1_cache = Linear.forward(out, W2, b2)
    ...
    dout, grads['W3'], grads['b3'] = Linear.backward(dscores, fc2_cache)
    dout = ReLU.backward(dout, relu_cache)
    dout, grads['W2'], grads['b2'] = Linear.backward(dout, fc1_cache)
    dout = dout.view(X.shape[0], W1.shape[0], X.shape[2] // 2, X.shape[3] // 2)
    dx, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(dout, conv_cache)
...
class DeepConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    for i in range(self.num_layers - 1):
        ...
        self.params[f'W{layer_index}'] = weight_initializer(C, F, K=3, dtype=dtype, device=device) if weight_initializer else weight_scale * torch.randn(F, C, 3, 3, dtype=dtype, device=device)
    ...
    self.params[f'W{self.num_layers}'] = weight_initializer(C * (H // (2 ** len(max_pools))) * (W // (2 ** len(max_pools))), num_classes, dtype=dtype, device=device) if weight_initializer else weight_scale * torch.randn(C * (H // (2 ** len(max_pools))) * (W // (2 ** len(max_pools))), num_classes, dtype=dtype, device=device)
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers - 1):
        ...
        if self.batchnorm:
            ...
            out, cache = Conv_BatchNorm_ReLU_Pool.forward(out, W, b, gamma, beta, conv_param, self.bn_params[i], pool_param) if i in self.max_pools else Conv_BatchNorm_ReLU.forward(out, W, b, gamma, beta, conv_param, self.bn_params[i])
        else:
            out, cache = Conv_ReLU_Pool.forward(out, W, b, conv_param, pool_param) if i in self.max_pools else Conv_ReLU.forward(out, W, b, conv_param)

        caches.append(cache)
    W = self.params[f'W{self.num_layers}']
    b = self.params[f'b{self.num_layers}']
    out, cache = Linear.forward(out.view(out.shape[0], -1), W, b)
    caches.append(cache)
    ...
    for i in range(self.num_layers - 1, 0, -1):
        ...
        if self.batchnorm:
            if i in self.max_pools:
                dout, grads[f'W{layer_index}'], grads[f'b{layer_index}'], grads[f'gamma{layer_index}'], grads[f'beta{layer_index}'] = Conv_BatchNorm_ReLU_Pool.backward(dout, caches.pop())
            else:
                dout, grads[f'W{layer_index}'], grads[f'b{layer_index}'], grads[f'gamma{layer_index}'], grads[f'beta{layer_index}'] = Conv_BatchNorm_ReLU.backward(dout, caches.pop())
        else:
            if i in self.max_pools:
                dout, grads[f'W{layer_index}'], grads[f'b{layer_index}'] = Conv_ReLU_Pool.backward(dout, caches.pop())
            else:
                dout, grads[f'W{layer_index}'], grads[f'b{layer_index}'] = Conv_ReLU.backward(dout, caches.pop())

        grads[f'W{layer_index}'] += 2 * self.reg * self.params[f'W{layer_index}']
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class BatchNorm(object):
  ...
  def backward(dout, cache):
    ...
    dvar = (dx_normalized * (x - mean) * -0.5 * (var + eps) ** (-1.5)).sum(dim=0)
    dmean = dx_normalized.sum(dim=0) * -1 / torch.sqrt(var + eps) + dvar * (x - mean).sum(dim=0) * -2 / N
    dx = dx_normalized / torch.sqrt(var + eps) + dvar * 2 * (x - mean) / N + dmean / N
