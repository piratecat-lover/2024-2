...
def compute_distances_one_loop(x_train, x_test):
  ...
  dists = x_train.new_zeros(num_train, num_test)
  for i in range(num_train):
    dists[i] = torch.sum((x_train[i].flatten() - x_test.flatten(1))**2, dim=1)
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  train_sqrt = torch.sum(x_train_flat**2, dim=1).reshape(-1, 1)
  test_sqrt = torch.sum(x_test_flat**2, dim=1).reshape(1, -1)
  dists = train_sqrt + test_sqrt - 2 * torch.mm(x_train_flat, x_test_flat.T)
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  for i in range(num_test):
    _, topk_idx = torch.topk(dists[:, i], k, largest=False)
    y_nearest = y_train[topk_idx]
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for x_chunk, y_chunk in zip(torch.chunk(x_train, num_folds), torch.chunk(y_train, num_folds)):
    x_train_folds.append(x_chunk)
    y_train_folds.append(y_chunk)
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
      ...
      accuracy = knn.check_accuracy(x_val_fold, y_val_fold, k=k, quiet=True)
...
def knn_get_best_k(k_to_accuracies):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  best_k = min(k_to_accuracies, key=lambda k: -statistics.mean(k_to_accuracies[k]))
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
  return best_k
