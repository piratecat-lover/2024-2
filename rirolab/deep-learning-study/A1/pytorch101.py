...
def create_sample_tensor():
  ...
  x = None
  x = torch.zeros(3,2)
  x[0:2,0:2] = torch.tensor([[0,100],[10,0]])
  return x
...
def multiples_of_ten(start, stop):
  ...
  else: x = torch.arange((start//10)*10,stop+1,10,dtype=torch.float64)
  return x
...
def zero_row_min(x):
  ...
  """
  y = x.clone()
  y.scatter_(1, x.argmin(dim=1, keepdim=True), 0)
  return y
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  """
  if use_loop:
    z = torch.stack([x[i].mm(y[i]) for i in range(x.size(0))])
  else: z = x.bmm(y)
  return z
...
def normalize_columns(x):
  ...
  var = ((x - mean)**2).sum(dim=0)/x.size(0)
  y = (x - mean)/var.sqrt()
  return y
