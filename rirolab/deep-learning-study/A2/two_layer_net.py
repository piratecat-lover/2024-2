...
def nn_forward_pass(params, X):
    ...
    hidden = torch.max(hidden, torch.tensor(0.0, dtype=hidden.dtype, device=hidden.device)) # ReLU
...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    grads['W2'] = h1.t().mm(dscores) + 2 * reg * W2 # Shape: (H, C)
    grads['b2'] = dscores.sum(dim=0) # Shape: (C,)
    
    dh1 = dscores.mm(W2.t()) # Shape: (N, H)
    dh1[h1 <= 0] = 0 # ReLU backprop
    
    grads['W1'] = X.t().mm(dh1) + 2 * reg * W1 # Shape: (D, H)
    grads['b1'] = dh1.sum(dim=0) # Shape: (H,)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_get_search_params():
  ...
  learning_rate_decays = [0.9, 0.99, 0.995, 0.999]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
      ...
      for hs in hidden_sizes:
          for reg in regularization_strengths:
              for lr_decay in learning_rate_decays:
                  # Initialize a new TwoLayerNet with current hyperparameters
                  net = TwoLayerNet(input_size=data_dict['X_train'].shape[1],
                                    hidden_size=hs,
                                    output_size=int(torch.max(data_dict['y_train']) + 1))
                  ...
                  print(f'lr: {lr}, hidden_size: {hs}, reg: {reg}, lr_decay: {lr_decay} => val_acc: {val_acc}')
  #############################################################################
  #                               END OF YOUR CODE                            #
  #############################################################################
